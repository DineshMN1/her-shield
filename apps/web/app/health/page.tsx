...
export default function HealthTrackingPage() {
  ...
  const fetchHealthData = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/health/metrics`,
        {
          headers: {
  ...
  const fetchConnectedDevices = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/health/devices`,
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const connectDevice = async (deviceType: string) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/health/devices/connect`,
        {
          method: 'POST',
          headers: {
